@ECHO OFF
	...
	FOR /D %%G IN (.\*) DO (
		...
		IF EXIST "manifest.xml" (
			pakman --compile --source=.\ --output=..\bin\org.santedb.sg.pak --keyFile=..\..\keys\community.santesuite.net.pfx --keyPassword=..\..\keys\community.santesuite.net.pass --embedcert --install
		)
		POPD
	)
